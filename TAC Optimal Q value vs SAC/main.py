...
memory=[mixed_memory1,mixed_memory2,mixed_memory3,mixed_memory4,mixed_memory5,mixed_memory6]
...
total_numstep=[total_numsteps1,total_numsteps2,total_numsteps3,total_numsteps4,total_numsteps5,total_numsteps6]
...
R_MAX=[R_MAX1,R_MAX2,R_MAX3,R_MAX4,R_MAX5,R_MAX6]
    ...
    for iteration in range(10):
        ...
        for i_episode in range(args.epoch):
            ...
            while not done:
                if args.render:
                    env.render()
                if args.start_steps>total_numstep[i]:
                    action=env.action_space.sample()
                else:
                    action=agent[i].select_action(state)

                if len(memory[i])>args.batch_size:

                    for s in range(args.updates_per_step):

                        agent[i].update_parameters(memory[i],args.batch_size,updates)
                ...
                memory[i].push(state, action, reward, next_state, mask)  # Append transition to memory
                transition_list.append([state, action, reward, next_state, mask])

                state=next_state

            if episode_reward>R_MAX[i]:
                R_MAX[i]=episode_reward
                for t in transition_list:
                    memory[i].hpush(*t)


            print("{}th Episode: {},total numsteps: {},episode steps: {},reward: {}".format(i,i_episode, total_numstep[i],
                                                                                       episode_steps,
                                                                                       round(episode_reward, 2)))
        reward_list[i]=round(episode_reward, 2)

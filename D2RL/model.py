...
class D2RL_QNetwork(nn.Module):
    def __init__(self, num_inputs, num_actions, hidden_dim):
        ...
        self.linear9 = nn.Linear(in_dim, hidden_dim)
        self.linear10 = nn.Linear(hidden_dim, 1)
...
class D2RL_GaussianPolicy(nn.Module):
    def __init__(self, num_inputs, num_actions, hidden_dim, action_space=None):
        super(D2RL_GaussianPolicy, self).__init__()
    ...
    def to(self, device):
        self.action_scale = self.action_scale.to(device)
        self.action_bias = self.action_bias.to(device)
        return super(D2RL_GaussianPolicy, self).to(device)


class Tsallis_GaussianPolicy(nn.Module):
    def __init__(self,num_inputs,num_actions,hidden_dim,action_space=None,q=1):
    ...
    def py_log_q(self,x,q):
        q=2-q
        q=float(q)
        value=torch.tensor([1e-6],dtype=torch.float32).cuda()
    ...
    def sample(self,state):
        ...
        log_prob=normal.log_prob(x_t)
        log_prob-=torch.log(self.action_scale*(1-y_t.pow(2))+epsilon)
        q_log_prob=self.py_log_q(log_prob.exp().float(),self.q)
...
class D2RL_Tsallis_GaussianPolicy(nn.Module):
    def __init__(self,num_inputs,num_actions,hidden_dim,action_space=None,q=1):
        super(D2RL_Tsallis_GaussianPolicy, self).__init__()
    ...
    def to(self,device):
        self.action_scale=self.action_scale.to(device)
        self.action_bias=self.action_bias.to(device)
        return super(D2RL_Tsallis_GaussianPolicy,self).to(device)